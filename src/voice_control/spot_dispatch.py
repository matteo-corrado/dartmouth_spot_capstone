...
def get_robot_state_dict() -> dict:
    ...
    # Map awareness — populate from map_loader API. Wrapped in try/except so a
    # NotImplementedError from the W1 stub or any I/O failure leaves the
    # defaults in place rather than blowing up the state collection.
    try:
        from pathlib import Path
        from src.map_loader import list_available_maps, read_last_used
        ...
        state["current_map"] = current or "unknown"
    except Exception:
        pass
...
def dispatch_intent(intent):
    ...
    try:
        ...
        if name == "stop":
            ...
            # Switch to a different GraphNav map (or reload last-used if no name).
            # Cancels any in-flight nav, uploads the requested graph + snapshots,
            # tries to fiducial-localize, and updates maps/.last_used. Failure
            # to localize is non-fatal — robot ends up "graph loaded, not
            # localized" and the user can drive in front of a fiducial and
            # retry. Speech response is generated by the LLM brain from the
            # action result; this handler only prints for the operator log.
            map_name_or_none = (params.get("map") or "").strip() or None
            try:
                from pathlib import Path
                from src.map_loader import resolve_map_path, write_last_used
                from src.graph_nav_utils import (
                    upload_graph_and_snapshots,
                ...
                # Cancel any in-progress nav before swapping the graph
                _cancel_nav()
                ...
                # Try to localize. Failure is non-fatal — robot is graph-loaded
                # but not localized; user can drive in front of a fiducial and
                # ask "load map" again to retry.
            ...
            # List the GraphNav maps available on disk and which one is loaded.
            try:
                from src.map_loader import list_available_maps, read_last_used
